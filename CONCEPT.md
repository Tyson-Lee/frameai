...
### 마일스톤 4 — 멀티 클라이언트 (MCP 서버화, ⚡ MVP 출시 완료)

> **부분 도착** — `scripts/frameai_mcp_server.py` 로 MCP **prompts**
> 인터페이스 노출 + `setup.sh` / `setup.ps1` 이 Claude Desktop 설정
> 파일에 자동 등록. 13 개 스킬 모두 Desktop 의 `/` 메뉴에서 즉시 사용
> 가능. **잔여 작업**: `tools/call` (deterministic 실행) 매핑, 훅
> 미들웨어 이전, Cursor/Continue 호환 검증.
...
- **MCP 서버화** — FrameAI 의 스킬·에이전트·훅 시스템을 MCP 의 tool /
  resource / prompt 인터페이스로 매핑. 한 번 만든 자동화가:
    ...
    - **Claude Desktop (MVP 완료 — `prompts/list` 노출, install 시 자동 config)**
    - ChatGPT 데스크톱 (MCP 클라이언트 모드 — 검증 미완)
    - Cursor / Continue / Cline 등 MCP 호환 IDE (검증 미완)
...
- **작업 규모**: 기존 SKILL.md 의 dual-mode 계약을 MCP `tools/list` +
  ...
  시스템 추상화. **prompts 매핑은 완료. tools/call (deterministic 실행)
  + 훅 이전은 2-4 주 추가 작업으로 추정**
